...
2. Open Enigma, scan devices, and connect directly to the ready USB entry.
3. Select a map point and test Set/Restore before using routes or joystick movement.

For wireless control, select **Enable desktop Wi-Fi** on the USB entry, put the Mac and
iPhone on the same normal Wi-Fi network, disconnect USB, scan again, and connect to the
Wi-Fi entry.

Review [compatibility](compatibility.md) before setup for the distinction between
software availability and the physically tested matrix.

## Optional embedded-board provisioning

Connect the supported Lichuang ESP32-S3 board to the Mac, then choose **Provision
embedded board** on any trusted, ready iPhone entry. Initial Apple trust still requires
USB, but provisioning can use either the USB or same-LAN Wi-Fi entry after pairing.
Keep the phone unlocked, approve Apple's modern pairing prompt, and close serial
monitors that own the CH340K port. The desktop transfers the pairing bundle directly
